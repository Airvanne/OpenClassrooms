class Personne
  ...
  def info
    ...
    if self.points_de_vie > 0
      "#{self.nom} (#{self.points_de_vie}/100 pv)"
    else
      "#{self.nom} (vaincu)"
    end
  end
  ...
  def subit_attaque(degats_recus)
    ...
    puts "#{self.nom} subit #{degats_recus}pv de dégâts !"
    if self.points_de_vie <= 0
      puts "#{self.nom} est donc vaincu"
    end
  end
end
...
class Joueur < Personne
  ...
  def soin
    ...
    # - Gagner de la vie
    # - Affiche ce qu'il s'est passé
    @points_de_vie = @points_de_vie + 13
    puts "#{nom} gagne de la vie ! (+ 13 pv)"
  end
  ...
  def ameliorer_degats
    ...
    puts "#{nom} améliore son attaque de 10 dégâts !"
  end
end
